# it is for basic implementtaion without simulation
...
# with simulation but it does not work for ports
...
from simulation import ping_ip  # Import the ping function from simulation.py
...
class FirewallGUI:
    ...
    def create_navbar(self):
        ...
        nav_buttons = [
            ...
            ("Apply Rules", self.apply_rules),
            ("Clear Rules", self.clear_rules),
            ("IP Simulation", self.show_ip_simulation_frame),  # New option for IP simulation
        ]
        for text, command in nav_buttons:
            btn = tk.Button(
                self.navbar,
    ...
    def show_ip_frame(self):
        """Show the IP management frame."""
        self.port_frame.pack_forget()
        self.ip_simulation_frame.pack_forget()  # Hide the simulation frame
        self.ip_frame.pack(fill="both", expand=True)
        self.update_ip_listbox()
    ...
    #     if ip:
    #         result = ping_ip(ip)
    #         self.sim_message_label.config(text=result, fg="green" if "established" in result else "red")
    #     else:
    #         messagebox.showwarning("Input Error", "Please enter an IP address to ping.")
            
            
    def ping_ip(self):
        """Ping an IP address and display results in the GUI."""
        ip = self.sim_ip_entry.get()  # Get IP from entry field
        if ip:
            # Call the simulation's ping_ip function
            result = ping_ip(ip)

            # Clear existing text in the simulation frame result area
            if hasattr(self, "sim_result_text"):
                self.sim_result_text.destroy()  # Remove the old result text box
            
            # Create a new text box for displaying results
            self.sim_result_text = tk.Text(self.ip_simulation_frame, wrap="word", height=15, bg="#f5f5f5")
            self.sim_result_text.insert(tk.END, result)
            self.sim_result_text.configure(state="disabled")  # Make text box read-only
            self.sim_result_text.pack(padx=10, pady=10, fill="both")
        else:
            messagebox.showwarning("Input Error", "Please enter an IP address to ping.")
